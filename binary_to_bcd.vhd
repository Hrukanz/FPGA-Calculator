...
-- Slight modifications made, start functionallity but was a left as a relic and for future reference
-- Instead the bin to BCD now operates when he binary input changes
-- Additionally the binary input is converted to its absolute value to account for negatives
...
begin
    ...
    process (clock, reset)
        ...
        variable bin_prev : std_logic_vector (bits - 1 downto 0);
            ...
            case state is
                ...
                when IDLE =>
                    if bin /= bin_prev then
                        ...
                        bin_reg := std_logic_vector(unsigned(abs(signed(bin))));
                        bin_prev := bin;
                        bcd_reg := (others => '0');
                        count   := 0;
                    end if;
